## 1. 项目简介
**`云知问答`** 是一个在线问答网站，类似于知乎、百度知道之类的网站。实现从人员注册、登录、权限管理、问题提问、回答、以及不同场景下的排序处理、统计分析、详情展示等；另外还有基于异步处理的短信验证、日志处理...子系统；管理员角色可通过后台管理模块进行系统管理。

当然啦，我们并不是想去做一个颠覆它们的网站，我们只是借鉴一下他们简洁优雅的界面以及问答风格，本质上还是学习实践成果。
